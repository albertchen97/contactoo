...
export default function Email({ emailIsOpen, setEmailIsOpen }) {
  ...
  return (
    ...
    <Modal
      ...
      overlayClassName={"z-30 flex justify-center items-center h-screen w-screen bg-black fixed top-0 bottom-0 left-0 right-0 bg-opacity-50 overflow-y-auto"}
      // styling for the actual modal window
      className={"absolute bg-white w-4/5 md:w-3/4 xl:2/3 z-40 max-w-4xl p-5 pt-20 rounded-2xl"}
      isOpen={emailIsOpen}
    ...
    >
        ...
        <div className="absolute top-10 right-5 md:right-10">
          <button className="text-3xl md:text-5xl" onClick={() => setEmailIsOpen(false)}>X</button>
        </div>

        {/* email form */}
        <form className="flex flex-col items-center justify-center w-full gap-5 text-xl md:text-2xl " onSubmit={sendEmail}>
